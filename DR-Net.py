...
class Network:
    def __init__(self, dataset, config, retrain=False):
        ...
        with tf.variable_scope('inputs'):
            ...
            self.inputs['neigh_idx_1'] = flat_inputs[2 * num_layers: 3 * num_layers]
            self.inputs['neigh_idx_2'] = flat_inputs[3 * num_layers: 4 * num_layers]
            self.inputs['sub_idx'] = flat_inputs[4 * num_layers:5 * num_layers]
            self.inputs['interp_idx'] = flat_inputs[5* num_layers:6 * num_layers]
            ...
            self.inputs['batch_xyz_anno'] = flat_inputs[6 * num_layers + 4]
            self.inputs['batch_label_anno'] = flat_inputs[6 * num_layers + 5]
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            xyz, neigh_idx, neigh_idx_1, neigh_idx_2, sub_idx = self.input_augment(is_training, inputs['xyz'][i], inputs['neigh_idx'][i],
                                                                                  inputs['neigh_idx_1'][i], inputs['neigh_idx_2'][i], inputs['sub_idx'][i])

            f_encoder_i = self.dilated_res_block(feature, xyz, neigh_idx, neigh_idx_1, neigh_idx_2, d_out[i],
        ...
        # ###########################Semantic Query############################
            ...
            f_interp.append(tf.expand_dims(interpolated_points, axis=2))
...
#         # feature = tf.expand_dims(feature, axis=2)
        ...
        for j in range(self.config.num_layers):
            f_decoder_i = tf_util.conv2d_transpose(tf.concat([f_interp[-j - 1], feature], axis=3),
                                                          f_interp[-j - 1].get_shape()[-1].value, [1, 1],
                                                          'Decoder_layer_' + str(j), [1, 1], 'VALID', bn=True,
                                                          is_training=is_training)
            feature = f_decoder_i
            f_decoder_list.append(f_decoder_i)
        # ###########################Decoder############################
        feature = tf.concat(f_decoder_list, axis=-1)

        interpolated_points = tf_util.conv2d(feature, 256, [1, 1], 'if_1', [1, 1], 'VALID', True,
                                             is_training)
    ...
    @staticmethod
    def input_augment(is_training, xyz, neigh_idx, neigh_idx_1, neigh_idx_2, sub_idx):
        xyz_c = tf.concat([xyz, xyz], axis=0)
        neigh_idx_c = tf.concat([neigh_idx, neigh_idx], axis=0)
        neigh_idx_c_1 = tf.concat([neigh_idx_1, neigh_idx_1], axis=0)
        neigh_idx_c_2 = tf.concat([neigh_idx_2, neigh_idx_2], axis=0)
        sub_idx_c = tf.concat([sub_idx, sub_idx], axis=0)
        # , neigh_idx_o, sub_idx_o
        element = tf.cond(is_training,
                          lambda: [xyz_c, neigh_idx_c, neigh_idx_c_1, neigh_idx_c_2, sub_idx_c],
                          lambda: [xyz, neigh_idx, neigh_idx_1, neigh_idx_2, sub_idx])
        return element
    ...
    def dilated_res_block(self, feature, xyz, neigh_idx, neigh_idx_1, neigh_idx_2, d_out, name, is_training):
        f_pc = tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_pc = self.building_block(xyz, f_pc, neigh_idx, neigh_idx_1, neigh_idx_2, d_out, name + 'LFA', is_training)
    ...
    def building_block(self, xyz, feature, neigh_idx, neigh_idx_1, neigh_idx_2, d_out, name, is_training):
        d_in = feature.get_shape()[-1].value

        f_xyz = self.relative_pos_encoding(xyz, neigh_idx)
        f_xyz = tf_util.conv2d(f_xyz, d_in, [1, 1], name + 'mlp0', [1, 1], 'VALID', True, is_training)
        f_neighbours = self.gather_neighbour(tf.squeeze(feature, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyz], axis=-1)
        f_pc_agg = self.att_pooling(f_concat, d_out // 4, name + 'att_pooling_0', is_training)

        f_xyz_1 = self.relative_pos_encoding(xyz, neigh_idx_1)
        f_xyz_1 = tf_util.conv2d(f_xyz_1, d_in, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_neighbours_1 = self.gather_neighbour(tf.squeeze(feature, axis=2), neigh_idx_1)
        f_concat_1 = tf.concat([f_neighbours_1, f_xyz_1], axis=-1)
        f_pc_agg_1 = self.att_pooling(f_concat_1, d_out // 4, name + 'att_pooling_1', is_training)


        f_add = tf.concat([f_pc_agg ,f_pc_agg_1], axis=-1)
        f_xyz = tf_util.conv2d(f_xyz, d_out // 2, [1, 1], name + 'mlp', [1, 1], 'VALID', True, is_training)
        f_neighbours = self.gather_neighbour(tf.squeeze(f_add, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyz], axis=-1)
    ...
    def relative_pos_encoding(self, xyz, neigh_idx):
        ...
        out_xyz= tf.concat([relative_dis, relative_xyz, xyz_tile, neighbor_xyz], axis=-1)
        return out_xyz
